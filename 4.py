...
## Save chi2 csv
    ...
    plt.savefig('./result/4_rf_selected.png')
    plt.close()
    ...
    result_df_2save = pd.DataFrame(data=data2save, columns=["index", "name", "importance"])
    result_df_2save.to_csv("./result/4_rf_selected.csv", index=False)

    ## Selected the best number of feature
    # rf = RandomForestClassifier(n_estimators=2000, n_jobs=-1)
    svc = SVC(kernel="linear")
    rfecv = RFECV(estimator=svc, step=500, cv=StratifiedKFold(y, 2),
              scoring='roc_auc')
    rfecv.fit(X, y)
